...
rosie_map_controller::objSrv;
rosie_map_controller::loadSrv;
...
//std::vector<rosie_map_controller::BatteryPosition> objStack.Batteries;
...
//			object.name = name;
		...
		if(obj_id != OBJECT){
			...
			float posX = evidence.object_location.x;
			float posY = evidence.object_location.y;
			float accuracy = 0.10; //5cm radius for faulty measurement
			bool pushed = 0;
...
//***********************************
			...
			if(collisionClient.call(collisionSrv)){
				if(collisionSrv.response.answer){
					//maybe provoke a stop driving
					mode = "toNext"; // the automatik procedure starts again, but leaves the wait state and goes back to the last action that is not yet completed
				}
			}
			if(newObstacle){
				mode = "toNext";
			...
			if(checkedStoredObjects){
				if(pathInitialized){ //this function should give 1 after it generated the path
					if(mode.compare("toNext"){
						rrtSrv.request.goalx = path[seq].x; //-1 means that the value of the MVO will be used
						rrtSrv.request.goaly = path[seq].y;
						rrtSrv.request.mode = 0; //go to most valuable object (MVO)
						mode = "wait"
					}else if(mode.compare("wait")){
						if (0 < (pow(pose.pose.pose.position.x-path[seq].x,2)+pow(pose.pose.pose.position.y-path[seq].y,2)) < 0.1*0.1){
							seq++;
						}
				...
				if(mode == "goto"){
					...
					if(rrtClien.call(rrtSrv)){
						goalX = rrtSrv.response.goalx;
						goalY = rrtSrv.response.goaly;
						mode = "wait";
						lastMode = "goto";
					}
				}
